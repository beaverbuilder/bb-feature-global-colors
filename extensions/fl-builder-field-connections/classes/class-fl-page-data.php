 ...
 * @since 1.0
 */
final class FLBuilderCoreFieldConnectionsData {
	...
	static public function add_site_property( $key, $data = array() ) {
		if ( class_exists( 'FLPageData' ) ) {
			FLPageData::add_site_property( $key, $data );
		}
	...
	static public function get_value( $object, $key, $settings = null ) {
		...
		if ( $property['form'] ) {
			...
			$value = call_user_func( $property['getter'], $key );
		}
...
FLBuilderCoreFieldConnectionsData::init();
